...
import {Selection} from 'app/content/services';
import {IssuesFilterMetadata} from 'app/content/services/requests-renderer/issues-filter-metadata';
...
export class RequestsList {
  ...
  requestsToDisplay = 20;

  requestFilterMetadata = IssuesFilterMetadata;

  @Input()
  set requestRendererOptionsState(state: RequestRendererOptionsState) {
  ...
  @Output()
  requestRendererOptionsChanged =
      new EventEmitter<RequestRendererOptionsState>();

  constructor(
      public requestsRenderer: RequestsRenderer, public cd: ChangeDetectorRef,
  ...
  ngOnInit() {
    ...
    // When request groups change, render the first ten, then debounce and
    // render more
    this.requestsRenderer.requestGroups.pipe(takeUntil(this.destroyed))
  ...
  renderNextGroup() {
    ...
    this.renderedRequestGroups.push({...nextRenderedRequestGroup, issues: []});
  }
  ...
  selectGroup(index: number) {
    this.selection.issues.select(
        ...this.requestGroups[index].issues.map(r => r.number));
  }
}
